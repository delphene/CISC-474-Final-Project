...
| <a name="check_end_m">check_end<a>  | checks after each action to see if the winner has changed | self, player, action | checks all winning combinations for [action](#action_v) (eg. if [action](#action_v) is 0 check top row, left column, -1 diagonal)<br>if [winner](#winner_p) is still None and there are no more [open](#open) spaces then game is tie <br>if x or o has won set reward |
