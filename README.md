...
| qlearning     | call qlearning algorithm | self, num_episodes | for each episode initialize [State](#States_p) to default Board value and set player to True (X)<br />while the [State](#States_p) has no [winner](#winner_p)<br />save a copy of the current state<br />choose an action for the current [player](#player_p), take that action on the board and swap the current [player](#player_p) to the other [player](#player_p)<br />if the new state after the action has not been seen add it to [states](#states_p) |
